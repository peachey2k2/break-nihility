...
var threshold: Vector4i = Decimal.from_float(1000.0)  # Default threshold: 1000
var decimal_places: int = 2  # Number of decimal places for abbreviated format
var thousands_separator: String = ","
var decimal_separator: String = "."
var show_abbreviation_space: bool = true  # Space between number and abbreviation
...
func format_abbreviated(decimal: Vector4i) -> String:
	...
	# But we can optimize: if exponent is >= 66, we know it's "c" or beyond
...
## Get the full number as a string with all zeroes (e.g., 43^32 -> string with all zeros)
func format_full_with_zeroes(decimal: Vector4i) -> String:
	if not Decimal.is_finite(decimal):
		return Decimal.to_string(decimal)
	...
	# Performance optimization: for very large numbers, use scientific notation
	# Building strings with thousands of zeros is extremely slow
	# Use a reasonable threshold (e.g., 100 digits = exponent ~100)
	if exponent > 100:
		return Decimal.to_exponential(decimal, decimal_places)
	...
	var total_mantissa_digits := integer_part.length() + fractional_part.length()
	...
	if decimal_position <= 0:
		...
		# Optimize: build zeros more efficiently for large counts
		var zeros_str := ""
		if leading_zeros_needed > 0:
			# For very large zero counts, use a more efficient approach
			if leading_zeros_needed > 1000:
				# Use scientific notation instead of building huge strings
				return Decimal.to_exponential(decimal, decimal_places)
		...
		# Optimize: avoid building huge zero strings
		if trailing_zeros_needed > 1000:
...
func add_thousands_separators(digits: String) -> String:
	...
	# Calculate how many separators we'll need
	var num_separators := (digits.length() - 1) / 3
	var result_length := digits.length() + num_separators * thousands_separator.length()
	
	# Build result more efficiently by calculating positions
	var result := ""
	var digit_idx := 0
...
func get_abbreviation_for_exponent(exp: int, use_long: bool = false) -> String:
	if exp in ABBREVIATIONS:
		return ABBREVIATIONS[exp][1 if use_long else 0]
	return ""
